    ...
    <nav role="navigation" aria-label="{{ __('Pagination Navigation') }}" class="flex items-center justify-center mt-10">
        <div class="flex items-center gap-2">
            ...
            @if ($paginator->onFirstPage())
                {{-- Kalau di halaman pertama (Mati/Redup) --}}
            ...
            @if ($paginator->hasMorePages())
                {{-- Kalau ada halaman selanjutnya (Aktif) --}}
            ...
            @else
                {{-- Kalau halaman terakhir (Mati/Redup) --}}
                <span class="px-3 py-2 text-green-200 bg-green-800/70 rounded-xl cursor-not-allowed opacity-50 border border-transparent flex items-center justify-center">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2.5" stroke="currentColor" class="w-5 h-5">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M8.25 4.5l7.5 7.5-7.5 7.5" />
                    </svg>
