...
<html lang="id">
  <head>
    ...
    <style>
        /* Style agar kotak pembayaran berubah warna saat dipilih */
        .payment-option.active {
            border-color: #166534; /* Green-700 */
  ...
  <body class="bg-[#F4F8F2] font-sans relative pb-20">
    ...
    <section class="max-w-6xl mx-auto px-6 mt-10">
      <div class="flex items-center justify-center space-x-6 text-sm font-medium text-gray-600">
        ...
        <div class="flex items-center space-x-2">
          <div class="w-6 h-6 rounded-full bg-gray-200 text-gray-500 flex items-center justify-center text-xs font-semibold">
            3
          </div>
        ...
        <div class="mb-8 border-b border-gray-100 pb-8">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <input type="text" value="{{ $data['nama'] }}" readonly class="w-full bg-gray-100 border border-gray-200 rounded-lg p-2.5 text-gray-600 text-sm focus:outline-none cursor-not-allowed">
                </div>
                {{-- WhatsApp --}}
                <div>
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea readonly class="w-full bg-gray-100 border border-gray-200 rounded-lg p-2.5 text-gray-600 text-sm focus:outline-none cursor-not-allowed" rows="2">{{ $data['alamat'] }}</textarea>
                </div>
                {{-- Catatan (Jika ada) --}}
                @if(!empty($data['catatan']))
                <div class="md:col-span-2">
                    <label class="block text-xs font-bold text-gray-500 mb-1">Catatan</label>
                    <input type="text" value="{{ $data['catatan'] }}" readonly class="w-full bg-gray-100 border border-gray-200 rounded-lg p-2.5 text-gray-600 text-sm focus:outline-none cursor-not-allowed">
                </div>
                @endif
            </div>
        </div>
        ...
        <form id="paymentForm" action="#" method="POST"> @csrf
            ...
            <div class="mb-8">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-3">
                    ...
                    <div class="payment-option cursor-pointer border border-gray-300 rounded-xl p-4 hover:border-green-500 transition flex items-center gap-3 relative overflow-hidden" onclick="selectPayment(this, 'Transfer Bank')">
                    ...
                    <div class="payment-option cursor-pointer border border-gray-300 rounded-xl p-4 hover:border-green-500 transition flex items-center gap-3 relative overflow-hidden" onclick="selectPayment(this, 'QRIS')">
                        <div class="w-8 h-8 rounded-full bg-gray-100 flex items-center justify-center text-gray-600">
                    ...
                    <div class="payment-option border border-gray-200 rounded-xl p-4 bg-gray-50 opacity-60 flex items-center gap-3 cursor-not-allowed">
            ...
            <div>
                ...
                <div class="flex gap-3">
                    <input type="text" name="voucher" placeholder="Contoh: HEMAT10" class="flex-1 border border-gray-300 rounded-lg p-3 placeholder-gray-400 focus:ring-2 focus:ring-green-500 focus:outline-none"/>
                    <button type="button" onclick="alert('Kode voucher berhasil digunakan! (Simulasi)')" class="px-5 py-3 bg-gray-100 border border-gray-300 rounded-lg font-bold text-gray-600 hover:bg-gray-200 transition">
                        Pakai
                    </button>
                </div>
            </div>
          ...
          <div class="space-y-4 text-gray-700">
            ...
            <div class="border-b border-gray-100 pb-4">
                <div class="flex justify-between items-start mb-2">
                    <span class="font-bold text-gray-800 text-lg">Paket {{ $paketOption->category->nama_kategori }}</span>
                </div>
            ...
            <div class="border-t border-gray-200 pt-4 flex justify-between items-center mt-2">
              ...
              <span class="font-extrabold text-[#1F5A34] text-xl">Rp {{ number_format($paketOption->harga, 0, ',', '.') }}</span>
            </div>
          </div>

          <button onclick="submitPayment()" class="block w-full mt-8 py-3.5 bg-yellow-400 hover:bg-yellow-600 text-black font-bold rounded-xl text-center transition shadow-md hover:shadow-lg transform hover:-translate-y-0.5 flex justify-center items-center gap-2">
    ...
    <div class="pointer-events-none fixed bottom-0 left-0 w-full h-full overflow-hidden z-0">
        <div class="absolute bottom-0 left-0 w-[220px] h-[220px] opacity-40 bg-no-repeat bg-contain" style="background-image:url('/asset/doodle-left.png');"></div>
        <div class="absolute bottom-0 left-0 w-[380px] h-[380px] opacity-40 bg-no-repeat bg-contain" style="background-image:url('/asset/kiri bawah.png'); background-position: bottom left; transform: translate(-10px, 10px);"></div>
        <div class="absolute bottom-[-50px] right-[-50px] w-[380px] h-[380px] opacity-40 bg-no-repeat bg-contain" style="background-image:url('/asset/kanan bawah.png');"></div>
    </div>

    <script>
        function selectPayment(element, value) {
            // 1. Reset semua pilihan jadi tampilan biasa
        ...
        function submitPayment() {
            ...
            if(!method) {
                ...
                // Scroll sedikit ke atas biar user liat errornya
                document.getElementById('paymentForm').scrollIntoView({behavior: "smooth", block: "center"});
                return;
            }
            
            // Jika sudah oke, Submit Form (Nanti diarahkan ke Controller Transaksi)
            // document.getElementById('paymentForm').submit(); 
            
            // Simulasi Sukses
            alert('Pesanan Dikonfirmasi! \nMetode: ' + method + '\nTotal: Rp {{ number_format($paketOption->harga, 0, ',', '.') }}');
        }
    </script>
