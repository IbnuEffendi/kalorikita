...
<body class="bg-green-700/60">
    ...
    @php
        ...
        // Placeholder sementara, nantinya diisi dari controller admin
        ...
        $latestOrders = $latestOrders ?? [];   // [{id, user_name, plan_name, total, status, created_at}, ...]
        $latestUsers  = $latestUsers ?? [];    // [{name, email, joined_at, active_plan}, ...]
        $topPackages  = $topPackages ?? [];    // [{name, active_subscribers, boxes_sent_today}, ...]
        $systemNotifications = $systemNotifications ?? []; // [{title, message, time}, ...]
        ...
        <div class="flex flex-col lg:flex-row gap-6">
            ...
            <aside class="w-full lg:w-64 bg-green-800/90 border border-green-700/70 rounded-3xl p-5 h-max">
                ...
                <nav class="space-y-2 text-sm">
                    ...
                    {{-- Connect Google (opsional, jika admin perlu) --}}
                    ...
                    {{-- Kembali ke tampilan user (opsional) --}}
                    <a href="{{ route('profil.dashboard') }}"
                       class="mt-3 flex items-center justify-between px-3 py-2 rounded-xl text-[11px] text-green-100 hover:bg-green-700/70">
            ...
            <main class="flex-1 space-y-6">
                ...
                {{-- SECTION ATAS: Profil admin + statistik hari ini --}}
                <section class="flex flex-col gap-6 lg:flex-row">
                    ...
                    <div
                        ...
                        {{-- Quick Actions Admin --}}
                        <div class="mt-6">
                            <p class="text-xs text-green-100/70 mb-2">Aksi Cepat Admin</p>
                    ...
                    {{-- Kolom kanan atas: Statistik ringkas hari ini --}}
                    <div class="w-full lg:w-80 flex flex-col gap-4">

                        {{-- Statistik Pesanan & Pendapatan Hari Ini --}}
                        <div class="bg-white rounded-3xl p-5 shadow-xl shadow-black/30 text-green-900">
                            <p class="text-xs font-semibold uppercase tracking-wide text-green-700 mb-1">Statistik Hari Ini
                            </p>
                            ...
                            <div class="space-y-3 mt-2 text-sm">
                                ...
                                <div class="flex items-center justify-between">
                                    ...
                                    <span class="font-extrabold">
                                        Rp {{ number_format($statsToday['revenue_today'] ?? 0, 0, ',', '.') }}
                                    </span>
                                </div>
                        ...
                        <div class="bg-green-800/90 border border-green-700/70 rounded-3xl p-4 shadow-lg">
                            <p class="text-xs text-green-100/80 mb-2 flex items-center justify-between">
                                ...
                                <span class="text-[10px] bg-green-700/70 rounded-full px-2 py-0.5">
                                    {{ count($systemNotifications) }} item
                                </span>
                            </p>

                            @if (empty($systemNotifications))
                                <p class="text-xs text-green-100/70">Tidak ada notifikasi sistem.</p>
                            @else
                                <ul class="space-y-2 max-h-32 overflow-y-auto pr-1">
                                    @foreach ($systemNotifications as $notif)
                                        <li class="text-xs text-green-100/80 bg-green-900/70 rounded-2xl px-3 py-2">
                                            ...
                                            @if (!empty($notif['time']))
                                                <p class="text-[10px] text-green-100/40 mt-1">
                                                    {{ $notif['time'] }}
                                                </p>
                                            @endif
                                        </li>
                ...
                {{-- GRID TENGAH: Paket populer + Ringkasan operasional --}}
                <section class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    ...
                    <div class="lg:col-span-2 bg-green-800/80 border border-green-700/60 rounded-3xl p-6 shadow-xl">
                        <div class="flex items-center justify-between mb-4">
                            <div>
                                <h2 class="text-lg font-semibold text-white">Paket Katering Aktif</h2>
                                <p class="text-xs text-green-100/70">
                                    Daftar paket dengan pelanggan aktif dan distribusi hari ini.
                                </p>
                            </div>
                            <a href="{{ route('admin.paket.index') }}"
                        ...
                        @if (empty($topPackages))
                            <div
                                ...
                                <p class="text-sm text-green-100/80 mb-2">
                                    Belum ada data paket aktif.
                                </p>
                                <p class="text-xs text-green-100/60 mb-4">
                                    Buat paket katering baru untuk mulai menerima pelanggan.
                                </p>
                                <a href="{{ route('admin.paket.create') }}"
                        ...
                        @else
                            <div class="space-y-3 max-h-72 overflow-y-auto pr-1 text-xs text-green-100/85">
                                @foreach ($topPackages as $pkg)
                                    <div
                                        class="bg-green-900/70 rounded-2xl px-4 py-3 flex items-center justify-between">
                                        <div>
                                            <p class="font-semibold text-white">
                                                {{ $pkg['name'] ?? 'Paket Katering' }}
                                            </p>
                                            <p class="text-[11px] text-green-100/60">
                                                Pelanggan aktif: {{ $pkg['active_subscribers'] ?? 0 }}
                                            </p>
                                        </div>
                                        <div class="text-right">
                                            <p class="text-[11px] text-green-100/60">Kotak terkirim hari ini</p>
                                            <p class="font-semibold text-yellow-300">
                                                {{ $pkg['boxes_sent_today'] ?? 0 }} box
                                            </p>
                                        </div>
                                    </div>
                                @endforeach
                    ...
                    {{-- Ringkasan operasional / log AI --}}
                    <div class="bg-green-800/80 border border-green-700/60 rounded-3xl p-6 shadow-xl flex flex-col">
                        <div class="flex items-center justify-between mb-3">
                            <div>
                                <h2 class="text-lg font-semibold text-white">Ringkasan KaloriLab (AI)</h2>
                                <p class="text-xs text-green-100/70">
                                    Aktivitas penggunaan AI oleh pengguna.
                                </p>
                            </div>
                        </div>

                        {{-- Placeholder, isi dari controller admin --}}
                        <div class="bg-green-900/70 rounded-2xl p-4 flex-1 flex flex-col justify-between text-xs text-green-100/85">
                            <ul class="space-y-1">
                                <li>
                                ...
                                <li>
                                    • Pengguna unik hari ini:
                                    <span class="font-semibold text-yellow-300">
                                        {{ $aiStats['unique_users_today'] ?? 0 }}
                                    </span>
                                </li>
                                <li>
                                    • Error / timeout:
                                    <span class="font-semibold text-yellow-300">
                                        {{ $aiStats['errors_today'] ?? 0 }}
                                    </span>
                                </li>
                            </ul>
                ...
                {{-- BOTTOM: Pesanan terbaru & Pengguna terbaru --}}
                <section class="grid grid-cols-1 lg:grid-cols-3 gap-6 mt-4">
                    ...
                    <div class="lg:col-span-2 bg-green-800/80 border border-green-700/60 rounded-3xl p-6 shadow-xl">
                        <div class="flex items-center justify-between mb-4">
                            <div>
                                ...
                                <p class="text-xs text-green-100/70">
                                    Monitor pesanan yang baru masuk.
                                </p>
                            </div>
                            <span class="text-[11px] text-green-100/70">
                                {{ count($latestOrders) }} pesanan
                            </span>
                        </div>
                                    ...
                                    <div
                                        ...
                                        <div class="text-right">
                                            ...
                                            <a href="{{ route('admin.orders.show', $order['id'] ?? 0) }}"
                                               class="text-[11px] text-green-100/70 underline">
                                                Lihat detail
                                            </a>
                                        </div>
                                    </div>
                    ...
                    <div class="bg-green-800/80 border border-green-700/60 rounded-3xl p-6 shadow-xl">
                        <div class="flex items-center justify-between mb-4">
                            <div>
                                <h2 class="text-lg font-semibold text-white">Pengguna Terbaru</h2>
                                <p class="text-xs text-green-100/70">
                                    Pantau pengguna baru yang mendaftar.
                                </p>
                            </div>
                        </div>
                                ...
                                @foreach ($latestUsers as $u)
                                    <li class="bg-green-900/70 rounded-2xl px-3 py-2">
                                        ...
                                        <p class="text-[10px] text-green-100/50 mt-1">
                                            Bergabung: {{ $u['joined_at'] ?? '-' }}
                                            @if (!empty($u['active_plan']))
                                                • Paket: {{ $u['active_plan'] }}
                                            @endif
                                        </p>
                                    </li>
